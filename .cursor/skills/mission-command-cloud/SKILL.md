...
| --- | --- | --- | --- | --- | --- | --- |
| <中文任务名> | 未开始 / 执行中 / 已完成待督察 / 督察通过 / 阻塞 / 待决策 | <一句话任务描述> | 0-100% | <中文分队代号> | <PR / artifacts 路径，无则「—」> | <阻塞原因或下一步动作，无则「—」> |

- 表格覆盖当前行动的**所有**任务（含 Subagent 与 Cloud Agent 分队），不得遗漏。
- 「任务名」与「负责分队」必须用中文表达（如「后端接口分队」「督察一队」）；分队的 unitId（bc-...）只在需要时以括号附注。
- 「任务完成进度」由参谋长综合分队战报与督察结论评定；仅分队自称完成、未经督察者，进度不得标为 100%。
- 表格之后再给态势要点、证据与决策请求。
