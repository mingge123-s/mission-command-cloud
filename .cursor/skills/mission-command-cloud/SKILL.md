...
| 任务名 | 任务状态 | 任务描述 | 任务完成等级 | 负责分队 | 证据链接 | 阻塞点/下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| <行动/分队任务名> | 未开始 / 执行中 / 已完成待督察 / 督察通过 / 阻塞 / 待决策 | <一句话任务描述> | S / A / B / C / F 或 0-100% | <Subagent / 分队 unitId 或代号> | <PR / artifacts 路径，无则「—」> | <阻塞原因或下一步动作，无则「—」> |

- 表格覆盖当前行动的**所有**任务（含 Subagent 与 Cloud Agent 分队），不得遗漏。
- 「任务完成等级」由参谋长综合分队战报与督察结论评定；仅分队自称完成、未经督察者，等级不得高于 B。
- 表格之后再给态势要点、证据与决策请求。
...
4. **强制汇报纪律**：任何任务完成（含 COMPLETE_CLAIMED、BLOCKED、COMMAND_DECISION）后，**必须第一时间向参谋长汇报**，不得静默收尾。调用回报协议（见下）；若参谋长 `agent_busy`，指数退避重试（脚本已内置），仍失败则把战报写入工作区约定路径并在自身最终回复中完整给出。未汇报视为任务未完成。
