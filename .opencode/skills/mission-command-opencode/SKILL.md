...
- 触发本 skill 即视为已授权创建独立会话：呈交编组后**直接创建分队会话**，无需再向统帅请求授权（统帅明确要求 dry run 除外）。
- **一律为具体执行工作创建独立分队会话**（含侦察、分析等只读工作），参谋长会话只做指挥与汇总，保持空闲以便统帅随时下达新任务；本会话 Subagent 仅用于几分钟内可完成的琐碎查证。
...
| 情况 | 编组 |
| --- | --- |
| 绝大多数任务（含只读侦察/分析） | 独立会话任务分队（可在 Web UI 查看，参谋长不占位） |
| 几分钟内可完成的琐碎查证 | 本会话 Subagent（task 工具） |
| 高风险验收 / 红队 | 独立会话督察 |
